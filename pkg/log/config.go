...
type Config struct {
	...
	Type string `json:"type" yaml:"type" env:"LogType"`
	// Level is the log level. Possible values are: debug, info, warn, error
	Level string `json:"level" yaml:"level" env:"LogLevel"`
	// File is the log file config.
	File *File `json:"file" yaml:"file" env:"LogFile"`
}
...
type File struct {
	...
	Path string `json:"path" yaml:"path" env:"LogFilePath"`
	// MaxSizeMB is the maximum size in megabytes of the log file before it gets rotated.
	MaxSizeMB int `json:"max_size_mb" yaml:"max_size_mb" env:"LogFileMaxSizeMB"`
	// MaxBackups is the maximum number of old log files to retain.
	MaxBackups int `json:"max_backups" yaml:"max_backups" env:"LogFileMaxBackups"`
}
